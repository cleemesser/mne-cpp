...
def parseInputArguments(argsToParse, **opts):
    ...
    for opt in inputOptions:
        key = opt[0] if caseSensitive else opt[0].lower()
        options[key] = opt[1]
    if mainMode:
        argsToParse2 = {}
    ...
    for arg in argsToParse:
        arg_adapted = arg if caseSensitive else arg.lower()
        if arg_adapted in options:
            options[arg_adapted] = argsToParse[arg]

        elif not relaxedMode:
...
def add_tracer_macro(in_text):
    ...
    if match := re.search(regExp, in_text):
        methodDef = noneIfEmpty(match["methodDef"])
...
def delete_tracer_macro(in_text):
    ...
    if match := re.search(regExp, in_text):
        methodDef = noneIfEmpty(match["trace"])
